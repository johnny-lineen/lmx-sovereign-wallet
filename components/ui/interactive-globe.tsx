...
export interface InteractiveGlobeProps {
  className?: string;
  /** Ignored when `fillContainer` is true—size the parent instead (e.g. Tailwind `w-60 aspect-square`). */
  size?: number;
  /** Canvas uses 100% width/height of its parent; set dimensions on the parent for responsive layouts. */
...
/** Rich mesh of routes between hubs — reads as global data / identity sync in motion. */
const DEFAULT_CONNECTIONS: { from: [number, number]; to: [number, number] }[] = [
  // Americas ↔ Europe / Atlantic
  ...
  { from: [37.78, -122.42], to: [-33.87, 151.21] },
  { from: [-23.55, -46.63], to: [1.35, 103.82] },
  { from: [-23.55, -46.63], to: [35.68, 139.69] },
  // Southern hemisphere / Oceania
  { from: [-33.87, 151.21], to: [-23.55, -46.63] },
  { from: [-33.87, 151.21], to: [1.35, 103.82] },
...
export function InteractiveGlobe({
  ...
  return (
    <canvas
      ref={canvasRef}
      className={cn("h-full w-full cursor-grab active:cursor-grabbing touch-none", className)}
      style={
        fillContainer
          ? { width: "100%", height: "100%", display: "block", maxWidth: "100%" }
...
/** Alias for demos that import `Component` from this module. */
